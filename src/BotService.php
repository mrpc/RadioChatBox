...
class BotService
{
    ...
    public function __construct(?SettingsService $settings = null, ?JobQueue $queue = null)
    {
    ...
    public function processReplyJob(array $payload): string
    {
        ...
        $llm = LlmService::fromSettings($this->settings);
        ...
        if (!$llm->isConfigured()) {
            ...
            $systemPrompt = self::buildSystemPrompt($fakeUser);
     ...
     *
     * A non-empty bot_custom_prompt replaces the generated prompt entirely;
     * bot_persona is appended to it.
     *
     * @param array<string,mixed> $fakeUser
     */
    public static function buildSystemPrompt(array $fakeUser): string
    {
    ...
    private function buildHistory(string $fakeNickname, string $peer, int $limit): array
    {
        ...
        // ...and end with it, otherwise there is nothing to reply to.
        while (!empty($history) && $history[count($history) - 1]['role'] === 'assistant') {
            array_pop($history);
        }
